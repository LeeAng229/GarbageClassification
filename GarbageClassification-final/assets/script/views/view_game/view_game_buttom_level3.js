...
cc.Class({
                ...
                if(this.speed){
                    if(this.usedGarbages[i].state === 'moving'){
                        ...
                        if(this.usedGarbages[i].x <= -800){
                            cc.log(this.usedGarbages[i],this.usedGarbages[i].x,"下传送带垃圾走出了屏幕外边");
                            //加载并播放错误的音效
                ...
                if(this.speed){
                    if(this.upUsedGarbages[i].state === 'moving'){
                        ...
                        if(this.upUsedGarbages[i].x >= 800){
                            cc.log(this.upUsedGarbages[i],"上传送带走出了屏幕外边");
                            //加载并播放错误的音效
                            loadSoundByPath([{clearWin:'musics/clearFail'}],'musics/clearFail');
                            //当垃圾走出屏幕后，给使用过的垃圾数量+1
                            this.usedGarbageNum  += 1;
    ...
    garbageTouchEnd(target){
        ...
        if(targetBustbin && targetBustbin.type === target.node.type){
            ...
            let boundY = GS.Random.getRandom(-324,-114);
            let targetId = target.node.id;
            for(let i in this.usedGarbages){
                if(targetId === this.usedGarbages[i].id){
                    target.node.position = cc.v2(900,-114);
                    this.unUsedGarbages.push(this.usedGarbages[i]);
                    this.usedGarbages.splice(i,1);
                }
            }
            ...
            for(let i in this.upUsedGarbages){
                if(targetId === this.upUsedGarbages[i].id){
                    target.node.position = cc.v2(-900,-12);
                    this.upUnUsedGarbages.push(this.upUsedGarbages[i]);
                    this.upUsedGarbages.splice(i,1);
                }
            }
        ...
        if(this.starNum == 1){
            ...
            gameLevelInfo[`level3`]['starNum'] = 1;
            GS.KVStorage.saveObj('GameLevelInfo',gameLevelInfo);
        }
    },
    ...
    robotAddScore(target){
        ...
        for(let i in this.usedGarbages){
            if(targetId === this.usedGarbages[i].id){
                cc.log('变更之前：',this.usedGarbages[i].x);
                target.position = cc.v2(900,-114);
                cc.log('变更之后：',this.usedGarbages[i].x);
                this.unUsedGarbages.push(this.usedGarbages[i]);
                this.usedGarbages.splice(i,1);
            }
        }
        ...
        for(let i in this.upUsedGarbages){
            if(targetId === this.upUsedGarbages[i].id){
                cc.log('变更之前：',this.upUsedGarbages[i].x);
                target.position = cc.v2(-900,-12);
                cc.log('变更之后：',this.upUsedGarbages[i].x);
                this.upUnUsedGarbages.push(this.upUsedGarbages[i]);
                this.upUsedGarbages.splice(i,1);
            }
        }
        ...
        this.usedGarbageNum += 1;
        //改变分数
        this.currentScore = this.currentScore = this.view_gameScene.getComponent('view_gameScene').setScore(10);
        if(this.starNum == 1){
            let gameLevelInfo = GS.KVStorage.loadObj('GameLevelInfo');
            gameLevelInfo[`level3`]['starNum'] = 1;
            GS.KVStorage.saveObj('GameLevelInfo',gameLevelInfo);
